...
class Codexin_Mailchimp_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$widget_ops = array(
			'classname' 	=> esc_attr( 'codexin-mailchimp-widget' ),
    ...
    public function enqueue_scripts() {
        ...
        wp_localize_script( 'codexin-mailchimp-script', 'cx_ajax_mailchimp', array(
	...
	public function form( $instance ) {
		...
		?>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'list' ) ); ?>"><?php echo esc_html__( 'Choose Mailchimp Email List:', 'codexin' ); ?></label>
			        ...
			        if ( isset ( $this->options['mc_api'] ) && ! empty ( $this->options['mc_api'] ) ) {
			            ...
			            if( $lists['data'] ) {
							...
							foreach( $mc_lists as $opt => $value ) {
								echo '<option value="' . esc_attr( $opt ) . '" id="' . esc_attr( $opt ) . '"', $list == $opt ? ' selected="selected"' : '', '>', esc_html( ucfirst( $value ) ), '</option>';
							}
			            } else {
			            	echo '<option id="no-list">' . esc_html__( 'No MailChimp Email Lists Found', 'codexin' ) . '</option>';
			            }

					} else {
						echo '<option id="invalid-mc-api">' . esc_html__( 'You have no Email Lists. Create an Email List to get started.', 'codexin' ) . '</option>';
					}
					?>
				</select>
			</p>
...
<?php
		...
		?>
			...
			<div class="codexin-mailchimp-wrapper" id="mailchimp-wrapper-<?php echo uniqid(); ?>">
			    ...
			    <form class="mailchimp-form clearfix" action="#" method="post">
			        ...
			        <div class="mailchimp-success"><?php echo esc_html( $msg_success ); ?></div>
	...
	<?php
			...
			echo '<p class="error">'. esc_html__( "No Valid MailChimp API Key found. Please make sure you have entered a Valid MailChimp API Key.", 'codexin' ).'</p>';
		}
