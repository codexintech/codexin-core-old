				...
				if( $data->have_posts() ) {
					...
					?>
						<article id="event-<?php echo esc_attr( get_the_ID() ); ?>" <?php post_class( array( esc_attr( $post_classes ) ) ); ?> itemscope itemtype="http://schema.org/Event">
						    <div class="<?php echo ( $layout == 'grid' ) ? esc_attr( 'portfolio-item-content' ) : esc_attr( 'post-wrapper reveal-border-1' ); ?>">
						    	<?php echo ( $layout == 'list' ) ? '<div class="port-list-wrapper reveal-bg-2">' : '' ?>
						    		<?php if( $layout == 'list' ) { ?>
						                <div class="thumb-port" style="background-image:url('<?php echo esc_url( $post_thumbnail ); ?>');">
						            ...
						            <div class="<?php echo ( $layout == 'list' ) ? esc_attr( 'desc-port' ) : esc_attr( 'portfolio-description' ) ?>">
						            	<?php 
						            	if( $layout == 'list' ) {
							            	if( ! empty( $port_list ) ) { ?>
